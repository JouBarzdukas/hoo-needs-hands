...
function App(): JSX.Element {
  ...
  const blobHighlightRef = useRef<HTMLDivElement | null>(null)
  const blobContainerRef = useRef<HTMLDivElement | null>(null)
  // Ref to track if we're currently processing a toggle
  const isTogglingRef = useRef(false)
  // Ref to always have the latest isListening value
  ...
  // Define addLogEntry BEFORE the SSE useEffect so that it is available in the callback.
  const addLogEntry = (type: 'input' | 'response', text: string) => {
    const newEntry = {
      id: Date.now(),
      type,
  ...
  // SSE connection: handle mic-on/off and log all other events
  useEffect(() => {
    const eventSource = new EventSource('http://localhost:8000/stream')
    ...
    eventSource.onmessage = (event) => {
      ...
      if (data === 'mic-on') {
        if (!isListeningRef.current) {
          startListening().catch((error) => console.error('Error starting listening:', error))
        }
      } else if (data === 'mic-off') {
        if (isListeningRef.current) {
          stopListening()
        }
      } else {
        // Append any other SSE events as log entries
  ...
  // Dynamic visual effects based on audio level
  useEffect(() => {
    if (!blobCoreRef.current) return

    if (isListening) {
      const brightness = 1.1 + Math.min(0.45, audioLevel * 0.006)
      const contrast = 1.2 + Math.min(0.45, audioLevel * 0.006)
      blobCoreRef.current.style.filter = `brightness(${brightness}) contrast(${contrast})`

      const blueIntensity = 0.4 + Math.min(0.45, audioLevel * 0.006)
      const purpleIntensity = 0.3 + Math.min(0.45, audioLevel * 0.006)
      const cyanIntensity = 0.4 + Math.min(0.45, audioLevel * 0.006)
      blobCoreRef.current.style.boxShadow = `
        0 0 10px rgba(0, 132, 255, ${blueIntensity}), 
        0 0 20px rgba(110, 47, 235, ${purpleIntensity}), 
        inset 0 0 15px rgba(0, 226, 226, ${cyanIntensity})
      ...
      if (blobEdgeRef.current) {
        const edgeOpacity = 0.2 + Math.min(0.35, audioLevel * 0.005)
        blobEdgeRef.current.style.opacity = edgeOpacity.toString()
        const rotationDuration = Math.max(12, 20 - audioLevel * 0.05)
        blobEdgeRef.current.style.animationDuration = `${rotationDuration}s`
      }
      ...
      if (blobEdgeRef.current) {
        blobEdgeRef.current.style.opacity = '0.2'
        blobEdgeRef.current.style.animationDuration = '20s'
      }
  ...
  // Update blob scaling based on audio level
  useEffect(() => {
    if (!blobRef.current) return

    if (isListening) {
      const scale = 1 + Math.min(0.25, audioLevel * 0.005)
      blobRef.current.style.transform = `scale(${scale})`
    } else {
      blobRef.current.style.transform = 'scale(1)'
    }
  ...
  // Manual microphone toggle (if needed)
  const toggleMicrophone = async () => {
    if (isListening) {
      stopListening()
    } else {
  ...
  const analyzeAudio = () => {
    ...
    const updateAudioLevel = () => {
      ...
      const avg = sum / dataArray.length
      const normalizedLevel = Math.min(100, Math.max(0, avg * 3.5))

      setAudioLevel((prevLevel) => {
        return normalizedLevel < prevLevel
          ? prevLevel * 0.75 + normalizedLevel * 0.25
          : prevLevel * 0.5 + normalizedLevel * 0.5
      })
